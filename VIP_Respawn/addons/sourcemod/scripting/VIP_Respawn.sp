/**
 * [VIP] Respawn (" ... L4D & L4D2 ... ")
 * 
 * Basado en uno de los trabajos de: R1KO
 * Editado por: BloodyBlade
 *
 * LICENCIA:
 * Este plugin fue publicado originalmente en HLMOD. Sin embargo su edición fue puesta en Github:
 * https://github.com/BloodyBlade/L4D2-Plugins/blob/main/VIP_Respawn_1.6/scripting/VIP_Respawn_1.6.sp
 *
...
#define LIMIT_MODE 			1 	//	Modo límite: 1 - X veces por ronda, 0 - X veces por mapa.

#define USE_AUTORESPAWN		1	//	¿Debería usar la función de reaparición automática?

#define RESPAWN_CMD 		"sm_respawn"	//	Comando de reaparición.

#define MIN_ALIVE_MODE		3 	//	Modo de funcionamiento sm_vip_respawn_min_alive:
								//		0 - Vivo en el equipo del jugador
								//		1 - Vivo en el equipo del jugador contrario
								//		2 - Número total de jugadores vivos en ambos equipos.
								//		3 - Vivos en cada equipo
...
int g_iMinAlive = 0;
bool g_bMusicEnabled = false;
bool g_bAddFileEnabled = false;
...
public void OnPluginStart()
{
	...
	BuildPath(Path_SM, sDataFilePath, sizeof(sDataFilePath), "data/vip/modules/Respawn.ini");
	if (!FileExists(sDataFilePath))
		SetFailState("[%T] '%s' File Not Found!", g_szFeature, sDataFilePath);
	
	ConVar hCvar = CreateConVar("vip_respawn_enable", "1", "Enable/Disable the plugin. \n0 = Disable, 1 = Enable.", FCVAR_NOTIFY, true, 0.0, true, 1.0);
	g_bEnabled = hCvar.BoolValue;
	hCvar.AddChangeHook(OnEnabledChange);

	hCvar = CreateConVar("vip_respawn_map_limit", "-1", "Respawn limit per round/map. \n-1 = No Limit, 0 = Prohibited.", FCVAR_NOTIFY, true, -1.0);
	g_iMapLimit = hCvar.IntValue;
	hCvar.AddChangeHook(OnMapLimitChange);

	#if defined GAME_CS || defined GAME_TF2 || defined GAME_DODS
	hCvar = CreateConVar("vip_respawn_end_duration", "0.0", "How many seconds after the start of the round a player can respawn. \n0.0 = Disabled.", FCVAR_NOTIFY, true, 0.0);
	hCvar.AddChangeHook(OnEndDurationChange);
	g_fEndDuration = hCvar.FloatValue;
	#endif

	hCvar = CreateConVar("vip_respawn_min_alive", "0", "What is the minimum number of living players a team must have for a player to be able to respawn. \n0 = Disabled.", FCVAR_NOTIFY, true, 0.0);
	hCvar.AddChangeHook(OnMinAliveChange);
	g_iMinAlive = hCvar.IntValue;

	hCvar = CreateConVar("vip_respawn_custom_music_enable", "0", "Enable/Disable the respawn custom music. \n0 = Disable, 1 = Enable.", FCVAR_NOTIFY, true, 0.0, true, 1.0);
	g_bMusicEnabled = hCvar.BoolValue;
	hCvar.AddChangeHook(OnEnableCustomMusic);

	hCvar = CreateConVar("vip_respawn_addfiletodownloads_enable", "0", "Enable/Disable AddFileToDownloadsTable. \n0 = Disable, 1 = Enable.", FCVAR_NOTIFY, true, 0.0, true, 1.0);
	g_bAddFileEnabled = hCvar.BoolValue;
	hCvar.AddChangeHook(OnEnableAddFile);
...
void OnEnableCustomMusic(ConVar hCvar, const char[] oldValue, const char[] newValue)	{ g_bMusicEnabled = hCvar.BoolValue; }
void OnEnableAddFile(ConVar hCvar, const char[] oldValue, const char[] newValue)	{ g_bAddFileEnabled = hCvar.BoolValue; }
...
stock bool CheckRespawn(int iClient, bool bNotify)
{
	...
	if(g_iMinAlive)
	{
		...
		#if MIN_ALIVE_MODE == 0			//	Vivo en el equipo del jugador
		if(iPlayers[iClientTeam == 2 ? 0:1] < g_iMinAlive)
		#elseif MIN_ALIVE_MODE == 1		//	Vivo en el equipo del jugador contrario
		if(iPlayers[iClientTeam == 2 ? 1:0] < g_iMinAlive)
		#elseif MIN_ALIVE_MODE == 2		//	Número total de jugadores vivos en ambos equipos.
		if(iPlayers[0] + iPlayers[1] < g_iMinAlive)
		#elseif MIN_ALIVE_MODE == 3		//	Vivos en cada equipo
...
void InitDataConfig()
{
	KeyValues kvData = new KeyValues("Respawn");
	...
	if (kvData.JumpToKey("Music"))
	{
		if (kvData.GotoFirstSubKey(false))
		{
			g_iTracks = 0; 
			
			do
			{
				if (g_iTracks >= sizeof(g_sTracks))
				{
					LogError("[%T] The maximum storage limit has been reached (%d songs).", g_szFeature, sizeof(g_sTracks));
					break;
				}
				
				kvData.GetString(NULL_STRING, g_sTracks[g_iTracks], sizeof(g_sTracks[]));
				
				if (g_sTracks[g_iTracks][0] != '\0')
				{
					PrecacheSound(g_sTracks[g_iTracks], true);
					if (g_bAddFileEnabled) AddFile(g_sTracks[g_iTracks]);
					g_iTracks++;
				}
				
			} while (kvData.GotoNextKey(false));
