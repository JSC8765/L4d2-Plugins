...
Handle g_hOnClientHatModelApplied = INVALID_HANDLE;
Handle g_hOnClientHatModelDestroyed = INVALID_HANDLE;
...
public APLRes AskPluginLoad2(Handle myself, bool late, char[] error, int err_max)
{
	...
	CreateNative("Hats_GetClientOverlayModel", GetOverlayHatModel);

	g_hOnClientHatModelApplied = CreateGlobalForward("Hats_OnClientModelApplied", ET_Event, Param_Cell, Param_Cell, Param_String);
	g_hOnClientHatModelDestroyed  = CreateGlobalForward("Hats_OnClientModelDestroyed", ET_Event, Param_Cell, Param_Cell);
...
int GetOverlayHatModel(Handle plugin, int numParams)
{
	if(numParams < 1)
	...
	if(!IsValidEntRef(g_iHatIndex[iClient]))
		return -1;
	
	return EntRefToEntIndex(g_iHatIndex[iClient]);
}
...
void RemoveHat(int client)
{
	...
	Call_StartForward(g_hOnClientHatModelDestroyed);
...
bool CreateHat(int client, int index = -1, bool notify = false)
{
	...
	if( entity != -1 )
	{
		...
		GetEntPropString(g_iHatIndex[client], Prop_Data, "m_ModelName", sModel, sizeof(sModel));

		CheckForSameModel(client, sModel);

		DispatchKeyValue(entity, "model", sModel);

		g_iHatIndex[client] = EntIndexToEntRef(entity);

		Call_StartForward(g_hOnClientHatModelApplied);
		Call_PushCell(client);
...
void CheckForSameModel(int iEntity, const char[] sPendingModel)// justincase 
{
	...
	PrintToServer("[HAT][%i]%s(NetClass) overlay_model is the same as base model! \"%s\"", iEntity, sNetClass, sModel);// used netclass because classname can be changed!
}
